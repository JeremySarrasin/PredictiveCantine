...
API_TOKEN = 'NjQzYWEwMTZiNjFjZWE3MGE0ZjQ4ZTYyOk5VNjVhcVBXWlBLckc5bUk0blQvVWRFY1I5eDNLSzV6dnVhZ1Q2WFFlK1E9'
PROJECT_ID_PARTICIPATION ='64520a140f9d9fd18087aeaa'
MODEL_ID_PARTICIPATION = '64521ce9c2ebe6159463f922'
PROJECT_ID_GASPILLAGE ='64521dc62d5c50a73687ae7d'
MODEL_ID_GASPILLAGE = '6452205c9ab7672aee5aedf9'
...
st.write("# Prédictive Cantine Promotion Lens")
	...
	for i in range(5):
		...
		for dish in ["Entrée", "Plat", "Légumes", "Dessert", "Laitage"]:
			...
			for comp in composants:
				...
				if match.shape[0] > 0:
					price += 0.15 * float(match.iloc[0]["Prix Unitaire Kg"].replace("€", ""))
...
with col2:
	...
	cols2_1_metrics[0].metric("Coût semaine par enfant", "{:2.1f}€".format(prix_semaine))
	cols2_1_metrics[1].metric("Coût total semaine", "{:2.1f}€".format(prix_semaine * num_students))
	total_etudiants = sum([x["Taux de participation"] for x in week_menus])
	cols2_1_metrics[2].metric("Coût total semaine optimisé", "{:2.1f}€".format(prix_semaine * num_students * total_etudiants / 5))
...
with col3:
	...
	cols3_1_metrics[0].metric("Coût total semaine", "{:2.1f}€".format(prix_semaine * num_students))
	cols3_1_metrics[1].metric("Économies réalisées", "{:2.1f}€".format(
		(np.asarray(gaspillage_initial) - np.asarray(gaspillage_prevu)).mean() * prix_semaine * num_students / 100
	))
	cols3_1_metrics[2].metric("Empreinte CO2 semaine", "{:2.1f} kg CO2".format(co2))
